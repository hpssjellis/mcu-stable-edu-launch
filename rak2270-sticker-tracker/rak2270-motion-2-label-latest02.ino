...
void setup() {
  ...
  delay(5000);
  Serial.println("You got 5 seconds to AT+BOOT if things screw up");
  delay(5000);
...
if (myCurrentIndex % NUMBER_OF_INPUTS == 0) {   // slows down showing the results
 ...
 // Serial.println("--" );  
  ...
  if (myPredict < 0.20) {  // if not label0] then something happened!
...
void mySendLoRaP2P() {
  // Define the payload
  uint8_t payload[] = "AFlipped over"; // Message to send. First letter defines image file name
  ...
  delay(20000); // 20000 is 20-second delay
}
